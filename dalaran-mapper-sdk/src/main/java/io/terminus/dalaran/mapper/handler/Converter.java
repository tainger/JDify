...
import io.terminus.dalaran.mapper.exception.FieldParseException;
...
public class Converter {
    ...
    public static Map<String, Object> convert(Object source, DalaranMappingConfig mappingConfig, DalaranFunctionContext dalaranContext) {
    ...
    private static void buildValue(Object source, Map<String, List<SourcePath>> sourcePaths, Map<String, PathDetail> destinationPaths, MessageMapping messageMapping, Object destination, DalaranFunctionContext dalaranContext) {
...
//    }
    ...
    private static void buildValue(Object source, MessageMapping messageMapping, Object destination, DalaranFunctionContext dalaranContext) {
        List<Object> values = new ArrayList<>();
        List<SourceField> sourceFields = messageMapping.getSourceFields();
...
//        Map<String, Object> contextValues = (Map<String, Object>)exchange.getProperties().get(DALARAN_CONTEXT_EXCHANGE + exchange.getExchangeId());
    ...
    private static Object execute(DalaranFunctionContext dalaranContext, MappingFunction function, List<Object> values) {
        try {
            switch (function.getType()) {
                case STATIC:
                    if (dalaranContext.getFunctionByKey(function.getId()).getContainsContext()) {
//                        values.add(new HashMap<>());
                    }
                    return dalaranContext.executeStaticFunction(function.getId(), values.toArray());
                case CUSTOM:
                    return dalaranContext.executeCustomFunction(Long.valueOf(function.getId()), values.toArray());
            }
            return null;
        } catch (Exception e) {
            e.printStackTrace();
            throw new RuntimeException("Mapper function execute error. function: " + function.getId() + ", params: " + Arrays.toString(values.toArray()) + ", message: " + e.getMessage());
        }
    }
}
