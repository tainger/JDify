...
public class DalaranAS2Server implements DalaranTrigger<AS2ServerConfig>, DalaranTriggerBuildAfterProcessor<AS2ServerConfig> {
    ...
    private static final String AS2_SERVER_URI = "as2://server/listen?requestUri=%s&serverPortNumber=%s";
...
//        String uri = String.format(AS2_SERVER_URI, config.getRequestUri(), config.getPort());
        route.from(uri);
