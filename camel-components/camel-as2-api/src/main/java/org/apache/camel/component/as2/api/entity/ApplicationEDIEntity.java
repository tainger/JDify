...
public abstract class ApplicationEDIEntity extends MimeEntity {
    ...
    public void writeTo(OutputStream outstream) throws IOException {
        ...
        try (CanonicalOutputStream canonicalOutstream = new CanonicalOutputStream(ncos, AS2Charset.US_ASCII);
            ...
            int length = ediMessage.length();
            byte[] bytes = ediMessage.getBytes(getCharset());
            if (Charsets.UTF_8.name().equalsIgnoreCase(getCharset())) {
                length = bytes.length;
            }
            transferEncodedStream.write(bytes, 0, length);
        } catch (Exception e) {
